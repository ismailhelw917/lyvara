...
async function main() {
  try {
    // Read the JSON file
    const jsonData = fs.readFileSync('/home/ubuntu/upload/pasted_content.txt', 'utf-8');
    const data = JSON.parse(jsonData);

    // Use category_results instead of products
    ...
    // Filter for jewelry products and extract data
    const products = [];
    const seenASINs = new Set();
    for (const product of sourceArray) {
      if (!product.asin || !product.image || !product.price) {
