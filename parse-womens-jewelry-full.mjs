...
async function main() {
  try {
    // Read the JSON file
    const jsonData = fs.readFileSync('/home/ubuntu/upload/pasted_content_5.txt', 'utf-8');
    const data = JSON.parse(jsonData);

    // Use category_results array from women's jewelry category
    ...
    console.log(`📦 Found ${sourceArray.length} women's jewelry products in JSON\n`);
    ...
    try {
      ...
      console.log(`\n🎉 Successfully inserted ${products.length} women's jewelry products!`);
    } finally {
